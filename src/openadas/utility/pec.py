...
def add_pec_thermal_cx_rate(donor_element, donor_charge, receiver_element, receiver_charge, transition, rate,
    ...
    for cls, elements in rates.items():
        for element, charge_states in elements.items():
            for charge, transitions in charge_states.items():
                ...
                path = os.path.join(repository_path, 'pec/{}/{}/{}.json'.format(cls, element, charge))
        ...
        for donor_charge, receiver_elements in donor_charge_states.items():
            for receiver_element, receiver_charge_states in receiver_elements.items():
                for receiver_charge, transitions in receiver_charge_states.items():
                    ...
                    rate_path = 'pec/thermal_cx/{0}/{1}/{2}/{3}.json'.format(donor_element, donor_charge,
                                                                             receiver_element,
                                                                             receiver_charge)
...
def _get_pec_rate(cls, element, charge, transition, repository_path=None):
    ...
    try:
        ...
        raise RuntimeError('Requested PEC rate (class={}, element={}, charge={}, transition={})'
                           ' is not available.'.format(cls, element, charge, transition))
...
def get_pec_thermal_cx_rate(donor_element, donor_charge, receiver_element, receiver_charge, transition,
                            repository_path=None):
    """
    ...
    try:
        ...
        raise RuntimeError(
            'Requested thermal charge-exchange PEC (donor={}, donor charge={}, receiver={}, receiver charge={})'
            ' is not available.'
            ''.format(donor_element, donor_charge, receiver_element, receiver_charge))
